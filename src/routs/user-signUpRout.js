...
async function createUser(req,res) {
  try{
  let {name,password,role} = req.body;//take the bode from the postman 
  let postUser = await user.create({name:name,password:await encrypt(password),role:role});//let new inside this var(imagin it like new row)
